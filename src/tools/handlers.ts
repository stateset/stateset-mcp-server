...
export async function handleToolCall(toolName: string, args: any): Promise<any> {
  switch (toolName) {
    ...
    case 'stateset_update_rma':
      throw new Error(
        'Updating returns is no longer supported. Use stateset_approve_return or stateset_restock_return.',
      );
    ...
    case 'stateset_update_shipment':
      throw new Error(
        'Shipment updates are not supported. Use stateset_mark_shipment_shipped or stateset_mark_shipment_delivered.',
      );
